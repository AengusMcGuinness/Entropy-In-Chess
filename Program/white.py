        ...
        with open(sys.argv[i]) as pgn:
            ...
            for moveNum, move in enumerate(game.mainline_moves()):
                if moveNum % 2 == 0:
                    ...
                    dataFile.write(f"Player: White Move Number: {moveNum/2} Move: {move} Piece: {piece} Number of Legal Moves: {numLegalMoves} Entropy: {shannonEntropy(numLegalMoves)} Conditional Entropy: {conditionalEntropy(numLegalMoves, newNumMovesCon)} Joint Entropy: {jointEntropy(numLegalMoves, newNumMovesCon)}\n")
