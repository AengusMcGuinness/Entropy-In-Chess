    ...
    for i in iterableFiles:
        with open(sys.argv[i]) as pgn:
            ...
            for moveNum, move in enumerate(game.mainline_moves()):
                ...
                #
                if ((numLegalMoves := len(list(board.legal_moves))) > 0):
                    info = engine.analyse(board, chess.engine.Limit(time=0.1, depth=10), multipv=numLegalMoves)
                    
                numBetterMoves = 1
                board.push(move)
                numLegalMoves = len(list(board.legal_moves))
                newInfo = engine.analyse(board, chess.engine.Limit(time=0.1, depth=10), multipv=numLegalMoves)
                newBetterMoves = 1
                if moveNum % 2 != 0:
                    dataFile.write(f"Player: Black Move Number: {moveNum//2} Move: {move} Piece: {piece} Number of Better Moves: {numBetterMoves} Entropy: {shannonEntropy(numBetterMoves)} Conditional Entropy: {conditionalEntropy(numBetterMoves, newBetterMoves)} Joint Entropy: {jointEntropy(numBetterMoves, newBetterMoves)}\n")
                    ...
                    totalJointEntropy.append(jointEntropy(numBetterMoves, newBetterMoves))
